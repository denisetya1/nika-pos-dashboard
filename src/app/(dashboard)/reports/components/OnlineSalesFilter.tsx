    ...
    <div className="flex flex-row justify-start gap-5 items-center mb-8">
      ...
      <div>
        <div className="mb-2 block">
          ...
          <Select onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setMarketPlace(e.target.value)} value={marketPlace}>
          <option value="">Semua Market Place</option>
          {marketPlaces.map((mp: string) => (<option key={mp} value={mp} >{mp}</option>))}
        </Select>
      </div>
