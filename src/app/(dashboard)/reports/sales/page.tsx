...
const StockReportPage = async ({
  ...
  const getFilterAndTotal = (data: Transaction[]) => {
    ...
    data.map((tr) => {
      ...
      if (totalByPayment[tr.outletPaymentMethod.paymentMethod.name]) {
        totalByPayment[tr.outletPaymentMethod.paymentMethod.name] =
          totalByPayment[tr.outletPaymentMethod.paymentMethod.name] +
          Number(tr.totalPrice);
      } else {
        payments.push(tr.outletPaymentMethod.paymentMethod.name);
        totalByPayment[tr.outletPaymentMethod.paymentMethod.name] = Number(
          tr.totalPrice,
        );
      }
    });
  ...
  const { shifts, users, payments } = getFilterAndTotal(transactions);
  ...
  const { totalAll: totalAllFiltered, totalByPayment: totalByPaymentFiltered } =
    getFilterAndTotal(filteredList);

  return (
    <div className="grow min-h-[500px]">
      <div>
        <h1 className="font-bold text-2xl mb-10">Laporan Penjualan Harian</h1>
      </div>
          ...
          <tbody className="divide-y">
              ...
              return (
                ...
                >
                  ...
                  <td className="px-6 py-3 w-[150px]">
                    {moment(tr.transactionTime).format("DD-MM-yyyy HH:mm:ss")}
                  </td>
                  ...
                  <td className="px-6 py-3 text-right text-black">
                    {formatCurrency(Number(tr.subTotal))}
                  </td>
                  <td className="px-6 py-3 text-right text-black">
                    {formatCurrency(Number(tr.totalDiscount))}
                  </td>
                  <td className="px-6 py-3 text-right text-black">
            ...
            {payments.map((p) => (
              <tr key={p as string} className="bg-slate-50">
                <td colSpan={9} className="px-6 py-3 text-right">
                  Total {p}
                </td>
