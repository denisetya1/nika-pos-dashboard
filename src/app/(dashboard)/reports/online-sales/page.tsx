...
const StockReportPage = async ({
  ...
  const getFilterAndTotal = (data: Transaction[]) => {
    ...
    data.map((tr) => {
      ...
      const findMpIndex = countByMarketPlace.findIndex(
        (item) => item.name === mp[0],
      );
      if (findMpIndex > -1) {
      ...
      const findCrIndex = countByCourier.findIndex(
        (item) => item.name === cr[0],
      );
      if (findCrIndex > -1) {
  ...
  const { totalAll: totalAllFiltered } = getFilterAndTotal(filteredList);

  return (
    <div className="grow min-h-[500px]">
      <div>
        <h1 className="font-bold text-2xl mb-10">
          Laporan Penjualan Online Harian
        </h1>
      </div>
        ...
        <div className="flex justify-between gap-10">
          <div className="w-1/2">
            <h2 className="text-md font-semibold">
              Ringkasan Berdasarkan Market Place
            </h2>
            <div>
              {countByMarketPlace.map(
                (item: { name: string; value: number }) => (
                  <div key={item.name} className="flex justify-start gap-4">
          ...
          <div className="w-1/2">
            <h2 className="text-md font-semibold">
              Ringkasan Berdasarkan Kurir
            </h2>
            <div>
              {countByCourier.map((item: { name: string; value: number }) => (
                <div key={item.name} className="flex justify-start gap-4">
                  <div className="w-[100px] p-1">{item.name}</div>
                  <div>: {item.value}</div>
                </div>
      ...
      <div className="bg-white border-[1px] border-slate-200 rounded-md overflow-hidden">
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
          ...
          <tbody className="divide-y">
            {filteredList.map((tr: Transaction, index) => {
              const mp = tr.cardNumber?.split("/") || [];
              const cr = tr.confirmNumber?.split("/") || [];
              ...
              return (
                ...
                >
                  ...
                  <td className="px-6 py-3 text-center">
                    {tr.user.name} ({tr.userShift.shift.name})
                  </td>
