          ...
          <tbody className="divide-y">
            {products.map((product, index) => (
              <tr key={product.id} className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                ...
                <td className="hidden sm:table-cell px-4 py-3 align-top">{product.stocks.length > 0 && `${product.stocks[0]?.markupPercentage}%`}</td>
                <td className="hidden sm:table-cell px-4 py-3 align-top">{product.stocks.length > 0 && `${product.stocks[0]?.discountPercentage}%`}</td>
