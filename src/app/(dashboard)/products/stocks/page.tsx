...
const ProductStock = async ({
  ...
  const outletId =
    searchParams?.outletId === undefined
      ? outlets[0].id.toString()
  ...
  const paginated: [Product[], number, number, number] =
    await resProduct.json();

  const displayLimit = 50;
  const [products, totalRow, currentPage] = paginated;
  const totalPages = Math.ceil(totalRow / displayLimit);

  const LitOutlets = await getOutletList();
  ...
  return (
    <div className="grow">
      ...
      <div className="border-[1px] border-slate-200 rounded-md">
        <Suspense key={currentPage} fallback={<p>Loading feed...</p>}>
            ...
            <thead>
              <tr className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400 border-top-[1px] border-slate-200 rounded-md">
                ...
                <th
                  ...
                  className="hidden sm:table-cell px-4 py-5 hover:bg-gray-200"
                >
                  <SortableHeader title="Stock" fieldName="quantity" />
                </th>
                <th
                  scope="col"
                  className="hidden sm:table-cell px-4 py-5 hover:bg-gray-200"
                ...
                >
                  ...
                  <td className="px-4 py-3 w-80 text-black dark:text-white">
                    ...
                    <div className="flex justify-start items-center gap-4 text-gray-500 text-xs align-top">
                      ...
                      <div className="flex text-gray-500 justify-start items-center gap-1">
                        <RiBarcodeBoxLine className="text-gray-400" />{" "}
                    ...
                    <div className="flex justify-end items-start align-top gap-2">
                      <div className="">
                        {product.stocks.length > 0
                          ? formatCurrency(Number(product.stocks[0]?.sellPrice))
                          : "-"}
                      </div>
                      <div className="">
                        <EditPriceForm
                  ...
                  <td className="hidden sm:table-cell px-4 py-3 align-top text-center">
                    {product.stocks.length > 0 &&
                    product.stocks[0]?.markupPercentage &&
                    product.stocks[0]?.markupPercentage > 0
                      ? `${product.stocks[0]?.markupPercentage}%`
                      : "-"}
                  </td>
                  <td className="hidden sm:table-cell px-4 py-3 align-top text-center">
                    {product.stocks.length > 0 &&
                    product.stocks[0]?.discountPercentage &&
                    product.stocks[0]?.discountPercentage > 0
                      ? `${product.stocks[0]?.discountPercentage}%`
                      : "-"}
                  </td>
                  <td className="hidden sm:table-cell px-4 py-3 align-top">
                    {product.stocks.length > 0 &&
                      !isEmptyVal(product.stocks[0].markupPercentage, true) &&
                      !isEmptyVal(
                        product.stocks[0]?.markupPercentage,
                        true,
                      ) && (
                  ...
                  <td className="px-4 py-3 text-center flex flex-col gap-2 justify-center items-center">
                    <StockTransferForm
                      product={product}
                      outlet={
                        outlets.filter((o) => o.id.toString() === outletId)[0]
                      }
                      currentQuantity={product.stocks[0]?.quantity}
                      outlets={LitOutlets}
                      productStock={product.stocks[0]}
